...
//	private static final String MAIN_MENU_OPTION_PROJECTS = "Projects";
	...
	private static final String[] MAIN_MENU_OPTIONS = new String[] {MAIN_MENU_OPTION_VENUES,
...
//																	 MAIN_MENU_OPTION_PROJECTS,
	...
	private void run() {
		...
		while(true) {
			...
			String choice = (String)menu.getChoiceFromMainMenu(MAIN_MENU_OPTIONS);
			if(choice.equals(MAIN_MENU_OPTION_VENUES)) {
				handleVenues();
			}
...
//			}
	...
	private void handleUpdateVenueName() {
		printHeading("Which venue would you like to view?");
		List<Department> allVenues = departmentDAO.getAllDepartments();
		if(allVenues.size() > 0) {
			Department selectedVenue = (Department)menu.getChoiceFromOptions(allVenues.toArray());
			String newVenueName = getUserInput("Enter new Department name");
			selectedVenue.setName(newVenueName);
			departmentDAO.saveDepartment(selectedVenue);
		} else {
			System.out.println("\n*** No results ***");
		}
	}
